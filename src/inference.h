...
class Inference {
   public:
    ...
    void setThresholds(float confidenceThreshold = 0.25,
                       float scoreThreshold = 0.45, float nmsThreshold = 0.50) {
   ...
   private:
    ...
    cv::Mat formatToSquare(const cv::Mat &source, int *pad_x, int *pad_y,
                           float *scale);
    ...
    std::vector<std::string> classes{"head"};
